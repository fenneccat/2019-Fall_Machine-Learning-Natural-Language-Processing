...
class BytePairEncoder:
    ...
    def _build_subword_units(self, vocabs):
        ...
        def get_stats(vocabs):
            ...
            for word, cnt in vocabs.items():
                subw = word.split(" ")
                for i in range(len(subw)-1):
                    pairs[(subw[i],subw[i+1])] += cnt
        ...
        def merge_vocab(pair, v_in):
            ...
            for key, value in v_in.items():
                if mpair in key:
                    nkey = key.replace(mpair, ''.join(pair))
                    v_out[nkey] = value
                
                else: v_out[key] = value
        ...
        for _ in trange(self.n_iters + 1):
            ...
            maxpair = max(pairs, key=lambda x: pairs.get(x))
            
            vocabs = merge_vocab(maxpair, vocabs)
...
def run(test_xs=None, test_ys=None, num_samples=10000, verbose=True,
    ...
    train_model(sess,
                ...
                batch_size=512, total_epoch=35, keep_prob_value=0.9)
