...
class SkipGram:
    ...
    def optimize(self, learning_rate, gradients, W, W_prime):
        ...
        for grad, center_i, context_i in gradients:
            self.W_prime[context_i] += learning_rate*grad*self.W[center_i]
            
        # Update W with W_prime_original and grad.
        for grad, center_i, context_i in gradients:
            self.W[center_i] += learning_rate*grad*W_prime_original[context_i]
    ...
    def subsampling(self, sample_bound, sentence):
        ...
        for word in sentence.strip().split(' '):
            f = self.vocab.word2count.get(word)
            index = self.vocab.word2index.get(word)
            if f is None:
                continue
            else:
                f = f/self.vocab.total_words
                p = (f-t)/f - math.sqrt(t/f)
                rand_num = np.random.random()
                
                if p > rand_num: continue
                else: sampled_word_indices.append(index)
