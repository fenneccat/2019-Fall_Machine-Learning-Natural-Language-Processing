...
def add_sentence_tokens(sentences, n):
    ...
    for sentence in sentences:
        wrap_sentence.append((SOS+' ')*n+sentence+(' '+EOS)*n)
    
    return wrap_sentence
...
def replace_unknown(tokens, cut_off):
    ...
    return [token if (token == SOS or token == EOS or fdist[token] > cut_off) else UNK for token in tokens]
...
class LanguageModel(object):
    ...
    def _convert_oov(self, ngram):
        ...
        for bitmask in self.masks:
            modi_ngram = mask(ngram, bitmask)
            if modi_ngram in self.model:
                return modi_ngram
    ...
    def perplexity(self, test):
        ...
        ## make cut_off to zero is more make sense but 1 can reproduce TA's result
        ...
        test_n_grams = nltk.ngrams(test_tokens, self.n)
        
        N = len(test_tokens)
        
        test_modi_n_grams = []
        for test_n_gram in test_n_grams:
            test_modi_n_grams.append(self._convert_oov(test_n_gram))
        
        pp = 0
        sum_log_prob = 0
        for m_token in test_modi_n_grams:
            prob = self.model[m_token]
            sum_log_prob += math.log(prob)
        pp = math.exp(-1/N * sum_log_prob)
    ...
    def _best_candidate(self, prev, i, without=None):
        ...
        def filterWithout(candy):

            if(candy[0] in without):
                return False
            else:
                return True

        candidates = filter(filterWithout, candidates)
        candidates = sorted(candidates, key=lambda x: x[1], reverse = True)
        #print("sorted candidates: ", [candy for candy in candidates][:5])
        
        #print(set(prev))
        if len(prev) == 0 or set(prev) == set([SOS]):
            return candidates[i]
