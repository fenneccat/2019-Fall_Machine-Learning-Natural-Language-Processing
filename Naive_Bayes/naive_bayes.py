...
class MyNaiveBayes:
    ...
    def fit(self, bows, labels):
        ...
        count_class = Counter(labels)
        for c in range(self.num_classes):
            self.class_to_num_sentences[c] = count_class[c]
            self.class_and_word_to_counts[c] = np.sum(bows[labels == c], axis=0)
    ...
    def get_prior(self):
        ...
        total = np.sum(self.class_to_num_sentences)
        
        return self.class_to_num_sentences/total
    ...
    def get_likelihood_with_smoothing(self):
        ...
        likelihood_with_smoothing = np.zeros((self.num_classes, self.num_vocab))
        
        self.class_and_word_to_counts += 1
        for c in range(self.num_classes):
            likelihood_with_smoothing[c] = self.class_and_word_to_counts[c]/np.sum(self.class_and_word_to_counts[c])
            
        return likelihood_with_smoothing
    ...
    def predict(self, bows):
        ...
        label_proba = np.zeros((self.num_classes, len(bows)))
        
        for c in range(self.num_classes):
            label_proba[c] = (np.dot(bows,self.log_likelihood[c])+self.log_prior[c])

        return np.argmax(label_proba, axis=0)
